...
class LocalVisionService:
    ...
    async def _make_vision_request(self, base64_image: str, prompt: str) -> Dict[str, Any]:
        ...
        if self.config.service_type == "ollama":
            ...
            # Prepare the request payload for LM Studio OpenAI-compatible API
            # Note: LM Studio doesn't support vision models in the same way
            # We'll use text completion with image description prompt
            payload = {
                "model": self.config.model_name,
                "messages": [
                    {
                        "role": "user",
                        "content": f"{prompt}\n\nNote: This is a text-only model. Please provide a general response about common objects that might be found in images for storytelling purposes."
                    }
                ],
                "temperature": 0.1,
                "max_tokens": 50,
    ...
    def _parse_vision_response(self, response_data: Dict[str, Any]) -> Tuple[Optional[str], Optional[float]]:
        ...
        try:
            ...
            if filtered_words:
                ...
                # For LM Studio (text-only), lower confidence since it's not actually seeing the image
                if self.config.service_type == "lm_studio":
                    confidence *= 0.6  # Reduce confidence for text-only responses
