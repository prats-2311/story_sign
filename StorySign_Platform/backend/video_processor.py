...
class PracticeSessionManager:
    ...
    def _trigger_signing_analysis(self) -> None:
        ...
        try:
            ...
            # Debug: Save landmark buffer to file for inspection
            try:
                import json, os
                file_path = os.path.join(os.path.dirname(__file__), "mediapipe_data_capture.json")
                with open(file_path, "w") as f:
                    json.dump(landmark_buffer, f, indent=2)
                self.logger.info(f"SUCCESS: Full landmark buffer saved to {file_path}")
            except Exception as save_err:
                self.logger.error(f"Could not save landmark buffer to file: {save_err}")
